...
{
    public class ResetPanelViewForm : BaseViewModel
    {
        ...
        public ResetPanelViewForm()
        {
            ...
            ResetCommand = new RelayParameterizedCommand(async (parameter) => await ResetPass(parameter));
        ...
        public async Task ResetPass(object parameter)
        {
            await RunCommand(() => this.LoginIsRunning, async () =>
            {
                ...
                else
                {
                    ...
                    @"(?(\[)(\[(\d{1,3}\.){3}\d{1,3}\])|(([0-9a-z][-\w]*[0-9a-z]*\.)+[a-z0-9]{2,17}))$"))
                    {
                        ...
                        {
                            ErrorVis = true;
                            ErrorText = "Ця пошта не використовується";
                            return;
                        }
                    }
                    else
                    ...
                    {
                        ErrorVis = true;
                        ErrorText = "Неправильний формат пароля";
                        return;
                    }

                    if (pass == passconf)
                    {
                        var result = entity.Users.SingleOrDefault(b => b.Email == email);
                        if (result != null)
                        {
                            string respass = crypt.Encrypt(pass);
                            if (respass == result.Pass)
                            {
                                ErrorVis = true;
                                ErrorText = "Не може зпівпадати з поточним";
                                return;
                            }
                            else
                            {
